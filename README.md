# Aditya Pandey
### AI/ML Researcher · Full-Stack Engineer · Applied Biometrics

<p align="center">
  <a href="https://twitter.com/thecritiz" target="_blank">
    <img src="https://img.shields.io/badge/Twitter-%40thecritiz-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white" alt="Twitter Badge" />
  </a>
  <a href="https://www.linkedin.com/in/thecritiz" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Aditya%20Pandey-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn Badge" />
  </a>
  <a href="https://github.com/thecritiz" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-thecritiz-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub Badge" />
  </a>
</p>

<p align="center">
  <img src="https://visitor-badge.la-roche.me/badge?page_id=thecritiz.thecritiz" alt="Profile views counter" />
</p>

---

## 🚀 About Me

A Computer Science enthusiast dedicated to building **human-centered AI systems**. My expertise lies at the intersection of **machine learning**, **biometric AI**, and developing **scalable full-stack applications**.

Currently exploring:

- Vision-based **attentiveness** and **emotion recognition** systems.
- Advanced **gait and behavioral biometrics** using point cloud and pose estimation techniques.
- Development of **privacy-aware interfaces** and robust research experimentation platforms.

---

## 🛠️ Technologies & Skills

<p align="center">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="32" style="background:white; border-radius:6px; padding:3px;" alt="Python" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" height="32" alt="Java" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="32" alt="JavaScript" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="32" alt="TypeScript" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" height="32" alt="C++" />
  <br />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="32" alt="React" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/express/express-original.svg" height="32" alt="Express.js" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="32" alt="PostgreSQL" />
  <br />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" height="32" alt="TensorFlow" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/keras/keras-original.svg" height="32" alt="Keras" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/opencv/opencv-original.svg" height="32" alt="OpenCV" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" height="32" alt="Pandas" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" height="32" alt="NumPy" />
  <br />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="32" alt="Git" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="32" alt="Docker" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" height="32" alt="Visual Studio Code" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" height="32" alt="Figma" />
  <br />
  <sub>**Note:** Some icons are black by default.</sub>
</p>

---

## 💻 Featured Projects

### **Expression Upgraded**
> Real-time facial emotion recognition using TensorFlow.js for enhanced user experiences.
> <br />
> 🛠️ **Tech:** `JS`, `TensorFlow.js`, `HTML5 Canvas`
> <br />
> 🔗 [GitHub](https://github.com/thecritiz/expression-upgraded)

---

### **Distraction Detect**
> Attentiveness monitoring using MediaPipe FaceMesh and OpenCV for focus analytics.
> <br />
> 🎯 **Applications:** Education, workplace monitoring
> <br />
> 🛠️ **Tech:** `Python`, `OpenCV`, `MediaPipe`, `Streamlit`
> <br />
> 🔗 [GitHub](https://github.com/thecritiz/distraction-detect)

---

### **Face Detection**
> Lightweight, frontend face detection using `face-api.js` for browser-based applications.
> <br />
> 📦 **Use Cases:** Demos, POCs, client-side security
> <br />
> 🛠️ **Tech:** `JS`, `HTML`, `face-api.js`
> <br />
> 🔗 [GitHub](https://github.com/thecritiz/Face_detection)

---

### **Portfolio**
> A sleek developer portfolio built with integrated project showcases and a blog.
> <br />
> 💡 **Purpose:** Personal branding and project presentation
> <br />
> 🛠️ **Tech:** `React`, `Tailwind CSS`, `Framer Motion`
> <br />
> 🔗 [GitHub](https://github.com/thecritiz/portfolio)

---

## 🔬 Ongoing Research

- **LidarGait++**: Point-transformer-based gait recognition using point cloud data.
- **Stylometric Authorship**: Detecting unique writing styles using NLP embeddings.
- **Chit-Chat**: A secure React Native chat platform with biometric authentication.
- **Portfolio OS**: A modular, interactive platform for research and product demonstrations.

---

## 📊 GitHub Insights

<p align="center">
  <img width="48%" src="https://github-readme-stats.vercel.app/api?username=thecritiz&show_icons=true&theme=tokyonight&hide_border=true&title_color=39C2C9&icon_color=39C2C9" alt="GitHub Stats" />
  <img width="48%" src="https://streak-stats.demolab.com?user=thecritiz&theme=tokyonight&hide_border=true&title_color=39C2C9&stroke=39C2C9" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=thecritiz&layout=compact&theme=tokyonight&hide_border=true&title_color=39C2C9&icon_color=39C2C9" alt="Top Languages" />
</p>

---

## 📬 Connect with Me

<p align="center">
  <a href="https://twitter.com/thecritiz" target="_blank">
    <img src="https://img.shields.io/badge/Twitter-%40thecritiz-1DA1F2?style=flat-square&logo=twitter&logoColor=white" alt="Twitter" />
  </a>
  &nbsp;
  <a href="https://www.linkedin.com/in/thecritiz" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-thecritiz-0A66C2?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  &nbsp;
  <a href="https://github.com/thecritiz" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-thecritiz-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub" />
  </a>
</p>
